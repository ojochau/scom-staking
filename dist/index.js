...
define("@scom/scom-staking/store/index.ts", ["require", "exports", "@scom/scom-staking/assets.ts", "@scom/scom-token-list", "@scom/scom-staking/global/index.ts", "@ijstech/components", "@scom/scom-staking/store/utils.ts"], function (require, exports, assets_1, scom_token_list_1, index_3, components_4, utils_1) {
    ...
    const getLockedTokenObject = (info, tokenInfo, tokenMap) => {
        if (info) {
            if (info.lockTokenType == index_3.LockTokenType.ERC20_Token) {
                if (!tokenMap) {
                    tokenMap = scom_token_list_1.tokenStore.tokenMap;
                }
    ...
    const getTokenDecimals = (address, chainId) => {
        ...
        const tokenObject = (!address || address.toLowerCase() === scom_token_list_1.WETHByChainId[chainId].address.toLowerCase()) ? ChainNativeToken : scom_token_list_1.tokenStore.tokenMap[address.toLowerCase()];
...
define("@scom/scom-staking/staking-utils/index.ts", ["require", "exports", "@ijstech/eth-wallet", "@scom/oswap-time-is-money-contract", "@scom/oswap-openswap-contract", "@scom/oswap-chainlink-contract", "@scom/oswap-cross-chain-bridge-contract", "@scom/scom-staking/store/index.ts", "@scom/scom-token-list"], function (require, exports, eth_wallet_4, oswap_time_is_money_contract_1, oswap_openswap_contract_1, oswap_chainlink_contract_1, oswap_cross_chain_bridge_contract_1, index_4, scom_token_list_2) {
                ...
                let promises = rewards.map(async (reward, index) => {
                    return new Promise(async (resolve, reject) => {
                        ...
                        try {
                            ...
                            initialReward = eth_wallet_4.Utils.fromDecimals(await rewardsContract.initialReward(), rewardTokenDecimals).toFixed();
    ...
    const composeCampaignInfoList = async (stakingCampaignInfoList, addDurationOption) => {
        ...
        for (let i = 0; i < _stakingCampaignInfoList.length; i++) {
            ...
            let campaignObj = Object.assign(Object.assign({}, stakingCampaignInfo), { campaignName: stakingCampaignInfo.name, campaignDesc: stakingCampaignInfo.desc, getTokenURL: stakingCampaignInfo.getTokenURL, options: durationOptionsWithExtendedInfo });
    ...
    const getAllCampaignsInfo = async (wallet, stakingInfo) => {
        let chainId = wallet.chainId;
    ...
    let ManageStake = class ManageStake extends components_6.Module {
        constructor(parent, options) {
            ...
            this.renderStakingInfo = async (info) => {
                ...
                if (tokenAddress && mode === 'Stake') {
                    if (lockTokenType == index_5.LockTokenType.ERC20_Token) {
                        ...
                        let balances = scom_token_list_3.tokenStore.tokenBalances;
            ...
            this.onSetupPage = async () => {
                ...
                this.tokenMap = scom_token_list_3.tokenStore.tokenMap;
                if (this.state.isRpcWalletConnected()) {
                    ...
                    command: (builder, userInputData) => {
                        ...
                        return {
                            execute: async () => {
                                ...
                                const { chainId, name, desc, logo, getTokenURL, showContractLink, stakings } = userInputData, themeSettings = __rest(userInputData, ["chainId", "name", "desc", "logo", "getTokenURL", "showContractLink", "stakings"]);
                            ...
                            if (o.mode === 'Stake') {
                                ...
                                if (btnStake) {
                                    ...
                                    if (inputElm) {
                                        ...
                                        const inputVal = new eth_wallet_6.BigNumber(inputElm.value || 0);
                    ...
                    const stakingsElm = await Promise.all(options.map(async (option, optionIdx) => {
                        ...
                        for (let idx = 0; idx < rewardsData.length; idx++) {
                            ...
                            const btnClaim = await components_9.Button.create({
                                ...
                                // background: { color: `${Theme.colors.primary.main} !important` },
                                // font: { color: Theme.colors.primary.contrastText },
                                enabled: !rpcWalletConnected || (rpcWalletConnected && !(!passClaimStartTime || new eth_wallet_6.BigNumber(reward.claimable).isZero()) && isClaim),
                        ...
                        const getAprValue = (rewardOption) => {
                            if (rewardOption && aprInfo && aprInfo[rewardOption.rewardTokenAddress]) {
                                const apr = new eth_wallet_6.BigNumber(aprInfo[rewardOption.rewardTokenAddress]).times(100).toFormat(2, eth_wallet_6.BigNumber.ROUND_DOWN);
                                return `${apr}%`;
                            }
                            return '';
                        };
